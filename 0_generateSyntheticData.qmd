...
```{r}
#| label: opts
#| code-fold-label: "Show options code"
...
#| code-fold-label: "Show library code"

library(pacman)

p_load(readxl, tidyverse, lubridate, hms, stringi, qualtRics,
       jsonlite, progress, sjlabelled, openxlsx)
```
...
#| label: sim-intake
#| output: false
#| code-fold-label: "Show Intake Code"
...
synPanel <- expand.grid(pid = as.character(1:N), wave = 1:panelWaves) |>
  ...
  # certain measures are only administered at certain waves, set these to 0 for the other waves
...
metaInfo <- tribble(
  ~Tab, ~Content,
  "Intake", "Data on participant demographics and gaming habits",
  "Panel", "Wellbeing, sleep and time use data from the panel survey",
  "Diary", "Daily diary data on gaming, wellbeing, and sleep",
  "Nintendo", "Telemetry data (session-level) from the Nintendo Switch",
  "Xbox", "Telemetry data (session-level) from Xbox devices",
  "Steam", "Telemetry data (total playtime in 1 hour blocks) from Steam",
  "iOS", "Screen time data from iOS devices, pulled from screenshots of the Screen time app",
  "Android", "App usage data from Android devices, pulled from ActivityWatch"
)
...
diaryCodebook <- synDiary |>
  ...
  mutate(
    Source = case_when(
      grepl("bangs", name) ~ "Basic Needs in Games Scale (https://doi.org/10.31234/osf.io/4965z7)",
      grepl("bpnsfs", name) ~ "Basic Psychological Need Satisfaction and Frustration Scale (https://doi.org/10.1007/s11031-014-9450-1)",
...
steamCodebook <- synSteam |>
  var_labels(
    ...
    genre = "The category or genre of the game played during the session (e.g., Action, Puzzle, Role-Playing).",
