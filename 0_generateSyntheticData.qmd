...
p_load(readxl, tidyverse, lubridate, hms, stringi, qualtRics,
       jsonlite, progress, sjlabelled, fixtuRes)
```
...
random_time <- function(n) {
  random_hours <- runif(n, 0, 24)
  random_minutes <- runif(n, 0, 60)
  random_times <- now() + dhours(random_hours) + dminutes(random_minutes)
  format(random_times, "%H:%M")
}
...
::: {.callout-note}
Some files are available publicly online, but others are private and only on OneDrive. The paths to these are specified in `.Renviron`. 
:::
...
intake <- fetch_survey(Sys.getenv("QUALTRICS_INTAKE_SURVEY_ID"), include_display_order = FALSE) |> 
  select(-(StartDate:consent), -(isWilling:RANDOM_ID), -gender_4_TEXT) |> 
  mutate(playProp_7 = "") # fix one broken column 
  
synIntake <- tibble(pid = as.character(1:N),
                    region = rep(c("US","UK"), N/2)) |> 
  # bootstrap each column from the non-NA values in the original data
  cbind(map_dfc(intake, ~rep(sample(.[!is.na(.)], length(.), replace = TRUE), length.out = N))) |> 

  # for a handful of relevant columns, we override the generate test response distributions with custom ones
  mutate(
    age = ifelse(region == "US", 
                 sample(18:30, N, replace = TRUE),
                 sample(18:75, N, replace = TRUE)),
    sex = sample(datDemo$sexProlific[!is.na(datDemo$sexProlific)], n(), replace = TRUE),
    employment = sample(datDemo$employmentStatusProlific[!is.na(datDemo$employmentStatusProlific) & 
...
dropout <- expand.grid(pid = 1:N, day = 1:studyDates) |> 
...
synNintendo <- synPlayHistory |> 
  ...
  mutate(
    ...
    sessionStart = date + seconds(runif(n(), 0, 86400)), # as_hms(sample(datNintendo$sessionStart, n(), replace = TRUE)),
...
synXbox <- synPlayHistory |> 
  ...
  unnest(session) |>
  mutate(titleID = sample(xboxGames$`Game Title`, n(), replace = TRUE),
...
synSteam <- synPlayHistory |> 
  ...
  mutate(
    sampled_row = list(sampleRowByID(persona, steam, numHoursWithSteamPlay)) # sample a certain number of hour-sessions
  ) |> 
...
## Two flavors of alternative code that create steam data, one without personas and one slower version, committing both for posterity
# # Get unique PIDs from synIntake
# steamPIDs <- unique(synIntake$pid[synIntake$playsSteam])
# numSteamDays <- 10
# 
...
#   total = length(steamPIDs), clear = FALSE, width = 60
# )
...
#   tmp <- steam %>%
#     group_modify(~ sample_n(.x, size = numSteamDays, replace = TRUE)) %>%
#     mutate(
#       PID = pid,
#       day = sample(1:studyDates, size = numSteamDays, replace = FALSE),
#       
#       # add days_since_study_start to studyStartDate
#       date = studyStartDate + days(daysSinceStart - 1)
#     )
#   
#   # Append the sampled data to the result tibble
#   synSteam <- bind_rows(synSteam, tmp)
#   
...
# synSteam <- synSteam |> 
#   select(-ID) |> 
#   rename(timeHH = time, minutesPlayed = minutes, titleID = Name) |> 
#   mutate(platform = "Steam") 

# synSteam <- synPlayHistory |> 
#   filter(numHoursWithSteamPlay > 0) |> 
#   mutate(hour = list(sample(0:24, numHoursWithSteamPlay))) |> 
#   unnest(hour) |>
#   mutate(
#     titleID = sample(steam$Name, n(), replace = TRUE),
#     duration = round(rbeta(n(), 3, 5)*60, 1),
#     genre = replicate(n(), assignGenres(uniqueGenres))
#   ) |> 
#   select(-(playsSwitch:numHoursWithSteamPlay)) |> 
#   arrange(pid, day, hour)
...
appCategories <- c("Entertainment", "Social", "Information & Reading", "Games", "Productivity & Finance", "Travel", "Other",
                "Creativity", "Education", "Health & Fitness", "Shopping & Food", "Utilities")

syniOS <- expand.grid(pid = synIntake$pid[synIntake$iOSuser], 
                      week = 1:(panelWaves*2),
...
synAndroid <- expand.grid(pid = synIntake$pid[synIntake$androidUser], 
                          day = 1:studyDays) |> 
  mutate(
...
panelClean <- panel |> 
  ...
  mutate(
    ...
    across(c(mctq_3_1, mctq_3_3, mctq_3_5, mctq_6_1, mctq_6_3, mctq_6_5), ~ random_time(n()))
  )

synPanel <- expand.grid(pid = as.character(1:N), wave = 1:panelWaves) |>
  mutate(date = studyDates[wave*14 - 13]) |> 
  
  # bootstrap each column from the non-NA values in the original data
  cbind(map_dfc(panelClean, ~rep(sample(.[!is.na(.)], length(.), replace = TRUE), length.out = N * panelWaves))) |> 
...
synDiary <- expand.grid(pid = synIntake$pid[synIntake$region == "US"], day = 1:diaryWaves) |>
  mutate(date = studyDates[day]) |> 
  # bootstrap each column from the non-NA values in the original data
  cbind(map_dfc(diary, ~rep(sample(.[!is.na(.)], length(.), replace = TRUE), 
  ...
  # remove data for all missing diary waves, and removing gaming data when the participant didn't play in the last 24 hours
  mutate(
...
Generate codebook.xlsx. Note that the cognitive task codebooks are in <https://github.com/digital-wellbeing/standard-stingray>.

```{r}
#| label: generate-codebook
#| eval: false

# TODO: finish codebook

metaInfo <- tribble(
  ~Tab, ~Content, ~Notes,
  "Nintendo", "Telemetry data", "",
  "Survey", "Wellbeing data", "Qualtrics survey data",
  "OSF", "Cognitive task data", "",
  "Demographics", "Demographic data from Prolific", "Some variables are similar to survey items; those in the demographics table are provided by Prolific, those in the survey are self-report and therefore are missing for participants who did not complete the survey"
)
...
diaryCodebook <- synDiary |>
  ...
  mutate(
    Source = case_when(
      ...
      grepl("trojan", name) ~ "Trojan Player Typologye (https://doi.org/10.1016/j.chb.2015.03.018)",
    ),
    `Response Options` = case_when(
      ...
      name %in% c("problematicPlay", "positives", "feedback") ~ "free text response",
...
write.xlsx(
  list(
    "Meta-info" = metaInfo, 
    "NoA" = noaCodebook, 
    "Qualtrics" = qualtricsCodebook, 
    "Demographics" = demoCodebook
  ),
  file = "codebook.xlsx",
...
setColWidths(codebook, "Qualtrics", cols = 1:3, widths = c(25, 100, 40))
addStyle(codebook, "Qualtrics", rows = nrow(qualtricsCodebook) + 3, cols = 1:3, style = createStyle(wrapText = TRUE))
...
saveWorkbook(codebook, paste0(Sys.getenv("DATA_PATH"), "/data-clean/codebook.xlsx"), overwrite = TRUE)
...
write_csv(synAndroid, "data-synthetic/synAndroid")
