...
data.panel <- data.table::fread("data-synthetic/synPanel.csv", na.strings = c("", "NA"))
read_csv("data-synthetic/synPanel.csv", 
                       name_repair = "universal",
         num_threads = 1,
         skip_empty_rows = TRUE,
         lazy = TRUE, 
         trim_ws = T) |> 
  select(psqi_6, starts_with("psqi"))

data.panel <- fread("data-synthetic/synPanelClean.csv") |> # requires that the preprocessing script has been run
  left_join(read_csv("data-synthetic/synIntakeClean.csv"), by = "pid") |> 
  select(pid, wave, psqi_6, starts_with(c("mctq", "eps", "psqi")), total_hours_sleep, wemwbs, 
...
gamingMonthly <- data.gaming |> 
  group_by(pid, month, latenight, isWeekend) %>%
  summarise(
    monthly_avg_minutes_played = sum(minutes_played)/28,
  ) |> 
  left_join(data.panel |> select(-msf_sc_numeric), by = c("pid","month" = "wave")) |> 
  ...
  filter(latenight == 1) |> 
  filter(monthly_avg_minutes_played  > 0) |> 
  ungroup() |> 
...
gamingBiweekly <- data.gaming |> 
  ...
  summarise(
    biweekly_avg_minutes_played = sum(minutes_played)/14,
  ) |> 
  left_join(data.panel, by = c("pid", "wave")) |> 
  filter(latenight == 1) |> 
  ungroup()
...
model.h1a <- clmm(psqi_6_ord ~ monthly_avg_minutes_played + (1 + monthly_avg_minutes_played | pid) +
                    age_scaled + bmi_scaled + SES_index_scaled + isWeekend,
                  data = gamingMonthly)
...
model.h1b <- lmer(total_hours_sleep ~ monthly_avg_minutes_played + (1 + monthly_avg_minutes_played | pid) +
              age_scaled + bmi_scaled + SES_index_scaled + (1 | gender) + isWeekend, data = gamingMonthly )
...
model.h1c <- lmer(wemwbs ~ biweekly_avg_minutes_played + (1 + biweekly_avg_minutes_played | pid) +
              age_scaled + bmi_scaled + SES_index_scaled + (1 | gender) + isWeekend, data = gamingBiweekly )
...
model.h2b <- lmer(total_hours_sleep ~ monthly_avg_minutes_played * msf_sc_numeric + (1 + monthly_avg_minutes_played | pid) +
                    age_scaled + bmi_scaled + SES_index_scaled + (1 | gender) + isWeekend, 
...
# Fit the model
model.h2c <- lmer(wemwbs ~ biweekly_avg_minutes_played * msf_sc_numeric + (1 + biweekly_avg_minutes_played | pid) +
                    age_scaled + bmi_scaled + SES_index_scaled + (1 | gender) + isWeekend, 
                  data = gamingBiweekly)
