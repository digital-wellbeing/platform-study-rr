...
We model this with a multilevel within-between logistic regression, where in-game need satisfaction (within- and between-person centered; `gameNS_cw` and `gameNS_cb`) predicts `playedAfterSurvey`, a binary variable indicating whether any play happened in the 24-hour period after diary survey completion.
...
plot_predictions(h2amod, condition = "gameNS_cw")
...
We model this with a multilevel within-between logistic regression, where in-game need satisfaction (within- and between-person centered; `globalNF_cw` and `globalNF_cb`) predicts `playedAfterSurvey`, a binary variable indicating whether any play happened in the 24-hour period after diary survey completion.

```{r}
#| label: h2b-model

h2bmod <- glmer(playedLaterAny ~ globalNF_cw + globalNF_cb + (1 + globalNF_cw|pid), 
               data = dat,
               family = binomial(link = "logit"))

plot_predictions(h2bmod, condition = "globalNF_cw")
...
h3mod <- lmer(globalNS ~ displacedCoreDomain + (1 + displacedCoreDomain|pid), data = dat)
