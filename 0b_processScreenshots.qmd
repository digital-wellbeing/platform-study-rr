...
screenshots <- list.files(path = Sys.getenv("screenshotsPath"), full.names = TRUE)
```
...
processText <- function(text) {
  text |> 
    ...
    filter(lag(value) %in% categories | row_number() == 1) |> # only keep the timestamps that match up with the categories
    mutate(
...
## Master function to run through the whole pipeline - processImage, OCR, processText
